...
> **모델 쪽 인계는 [HANDOFF-basemodel.md](HANDOFF-basemodel.md) 에 따로 있다** (2026-09-10).
> 이 문서는 **데이터 백필(2004~2009)** 작업 인계다.

**갱신 2026-09-07** — API 상태를 다시 실측해서 반영했다. 아래 코드블록을 통째로 복붙하면 된다.

> **갱신 2026-09-15 — 이 문서(2004~2009 백필)는 이미 닫혔다.**
> 백필은 했고 효과가 없었다(커밋 99a1dae · McNemar p=0.868 · ANOVA 데이터셋 기여 0.0%).
> **이 지시서대로 다시 하지 마라.** 아래 §1~6 은 그때의 절차 기록으로만 남긴다.
>
> 현재 작업 상태는 세 곳에 있다:
> - [HANDOFF-basemodel.md](HANDOFF-basemodel.md) §3-B·§3-C — 모델·제품 연동 진행분
> - [README.md](README.md) §11 — 제품 연동 실측 요약
> - 정원님 레포 `basemodel/AI-CONTRACT.md` — 백/프런트 구현 명세
>
> 아래 "현재 성능 기준선" 표도 옛 측정이다 — 데이터셋과 피처 구성이 그 뒤로 바뀌었다.
> 지금 기준선은 HANDOFF-basemodel.md §3-C 를 봐라.
